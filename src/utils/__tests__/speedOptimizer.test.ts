...
// Exercises the real production formula (utils/stats.ts calculateStat), the
// same formula SpeedOptimizer.tsx uses to compute speed spreads. Previously
// this suite reimplemented the formula locally and tested only itself.

const PLUS_SPEED = NATURES.find((n) => n.name === "jolly") as Nature; // +Spe / -SpAtk
const NEUTRAL = NATURES.find((n) => n.name === "docile") as Nature; // no change
const MINUS_SPEED = NATURES.find((n) => n.name === "brave") as Nature; // -Spe / +Atk

function calcSpeed(baseSpe: number, iv: number, ev: number, nature: Nature, level: number = 50): number {
  return calculateStat(baseSpe, iv, ev, nature, "speed", level);
}

function findMinEvs(baseSpe: number, iv: number, nature: Nature, targetSpeed: number): number | null {
  for (let ev = 0; ev <= 252; ev += 4) {
    if (calcSpeed(baseSpe, iv, ev, nature) >= targetSpeed) return ev;
  }
  return null;
}
...
describe("speed calculation", () => {
  it("calculates max speed for base 100 with +Spe nature", () => {
    ...
    const result = calcSpeed(100, 31, 252, PLUS_SPEED);
  ...
  it("calculates max speed for base 130 with +Spe nature", () => {
    ...
    const result = calcSpeed(130, 31, 252, PLUS_SPEED);
  ...
  it("calculates speed with 0 EVs", () => {
    ...
    const result = calcSpeed(100, 31, 0, NEUTRAL);
  ...
  it("calculates speed with negative nature", () => {
    ...
    const result = calcSpeed(100, 31, 252, MINUS_SPEED);
...
describe("findMinEvs", () => {
  ...
  it("returns null when target is unreachable", () => {
    ...
    const evs = findMinEvs(20, 31, NEUTRAL, 320);
    expect(evs).toBeNull();
  });

  it("returns exact EVs needed for neutral nature outspeed", () => {
    // Garchomp base 102, neutral, outspeed base 80 max neutral (Dragonite)
    // Max Dragonite neutral: calcSpeed(80, 31, 252, neutral)
    const dragoniteMax = calcSpeed(80, 31, 252, NEUTRAL);
    const evs = findMinEvs(102, 31, NEUTRAL, dragoniteMax + 1);
    expect(evs).not.toBeNull();
    expect(calcSpeed(102, 31, evs!, NEUTRAL)).toBeGreaterThanOrEqual(dragoniteMax + 1);
  });

  it("EVs are always multiples of 4", () => {
    const evs = findMinEvs(80, 31, PLUS_SPEED, 150);
    if (evs !== null) {
      expect(evs % 4).toBe(0);
    }
  });
