...
function onlineReducer(state: OnlineState, action: OnlineAction): OnlineState {
  switch (action.type) {
    ...
    case "RESET":
      return initialState;
    case "RNG_SEED":
      return { ...state, rngSeed: action.seed };
...
// One-time, non-deterministic seed generated by the host and shared with the guest —
// this does NOT need to be reproducible itself, only the per-turn battleRandom() stream
// derived from it (see battleRng.ts) needs to be identical on both clients.
function generateRngSeed(): number {
...
export function useOnlineBattle() {
  ...
  const setupConnection = useCallback((conn: import("peerjs").DataConnection) => {
    ...
    conn.on("data", (raw) => {
      ...
      switch (msg.type) {
        ...
        case "READY": {
          // Host's READY carries the shared battle RNG seed; guest's READY has none.
          const readyPayload = msg.payload as { rngSeed?: unknown } | null;
          const seed = readyPayload ? validateRngSeed(readyPayload.rngSeed) : null;
          if (seed !== null) {
            dispatch({ type: "RNG_SEED", seed });
          }
  ...
  const sendReady = useCallback(() => {
    if (!connRef.current?.open) return;
    // Only the host generates the shared battle RNG seed — the guest waits to receive
    // it in the host's READY message (see the "READY" case in setupConnection above).
    let payload: { rngSeed: number } | null = null;
    if (stateRef.current.isHost) {
      const seed = generateRngSeed();
      payload = { rngSeed: seed };
      dispatch({ type: "RNG_SEED", seed });
    }
    connRef.current.send({
      type: "READY",
