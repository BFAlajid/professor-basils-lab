...
export default function UnifiedEmulatorTab() {
  ...
  const handleBack = useCallback(() => {
    setMode("select");
    // Don't clear romFile — emulator keeps running in background
  }, []);
  ...
  return (
    <div>
      {/* ROM type selector — shown when mode is "select" */}
      {mode === "select" && (
      ...
      {/* Emulator components — stay mounted once activated, hidden when not selected */}
      {mountedModes.has("gba") && (
        <div className={mode === "gba" ? "block" : "hidden"}>
          {backButton}
          <GBAEmulatorTab initialFile={mode === "gba" ? romFile : undefined} />
        </div>
      )}

      {mountedModes.has("nds") && (
        <div className={mode === "nds" ? "block" : "hidden"}>
          {backButton}
          <NDSEmulatorTab initialFile={mode === "nds" ? romFile : undefined} />
        </div>
      )}

      {mountedModes.has("ctr") && (
        <div className={mode === "ctr" ? "block" : "hidden"}>
          {backButton}
          <CitrineEmulatorTab initialFile={mode === "ctr" ? romFile : undefined} />
        </div>
      )}
    </div>
  );
