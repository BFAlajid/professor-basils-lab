...
export default function AreaDetail({ area, onStartEncounter, isLoading }: AreaDetailProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-[#262b44] border border-[#3a4466] rounded-xl p-5 space-y-3"
    >
      <div>
      ...
      <div className="space-y-1.5">
        <h4 className="text-xs font-pixel text-[#8b9bb4] uppercase">Wild Pokemon</h4>
        <div className="grid grid-cols-2 gap-1.5">
          {area.encounterPool.map((entry) => {
            ...
            return (
              <div
                key={entry.pokemonId}
                className="flex items-center gap-2 bg-[#1a1c2c] rounded-lg px-3 py-2 border border-[#3a4466]"
              >
                <Image
                ...
                <div className="min-w-0">
                  <p className="text-xs text-[#f0f0e8] truncate">#{entry.pokemonId}</p>
      ...
      <button
        ...
        disabled={isLoading}
        className="w-full py-3 bg-[#38b764] hover:bg-[#2d9550] disabled:bg-[#3a4466] text-[#f0f0e8] text-sm font-pixel rounded-lg transition-colors"
      >
