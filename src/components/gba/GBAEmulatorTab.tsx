...
export default function GBAEmulatorTab(_props: GBAEmulatorTabProps) {
          ...
          {!state.isRunning && state.isReady && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 gap-4">
              <div className="text-center space-y-2">
                ...
                <p className="text-[#8b9bb4] text-xs">or</p>
                <button
                  onClick={() => romInputRef.current?.click()}
                  className="px-4 py-2 rounded-lg bg-[#e8433f] text-[#f0f0e8] text-xs font-pixel hover:bg-[#f05050] transition-colors"
                >
                  Choose ROM File
                </button>
              </div>
